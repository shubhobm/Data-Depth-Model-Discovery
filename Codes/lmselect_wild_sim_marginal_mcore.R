...
get.err = function(n.sig, X, sig, mseq, nboot, n.iter, ncores, corr=F){
  ...
  loopfun = function(m){
    ...
    for(iter in 1:n.iter){
      ...
      for(i in 1:nboot){
        iresid = as.matrix(sd * (rnorm(n)) * r, ncol=1)
        beta.mat[i,] = as.numeric(beta.hat) + as.numeric(H %*% iresid)
        
        samp = sample(1:n, m, replace=F)
        #iresid = as.matrix(sd * (rnorm(n)) * r, ncol=1)
        #beta.mat.m[i,] = as.numeric(beta.hat) + as.numeric(H[,samp] %*% iresid[samp])
        beta.mat.m[i,] = as.numeric(beta.hat) + as.numeric(H %*% iresid)*sqrt(n/m)
      }
      SSPmat.d[,p+1] = mdepth.TD(beta.mat.m, beta.mat.m)$dep
      
      # marginal models
      # do another moon wild bootstrap to get them
      #print("Do Monte-Carlo simulation")
      for(j in 1:p){
        
        jbeta.mat = beta.mat.m      
        if(corr==T){
          jbeta.mat[,j] = (1+sd)*beta.hat[j]
        }
        else{
          jbeta.mat[,j] = 0
        }
        
        SSPmat.d[,j] = mdepth.TD(jbeta.mat, beta.mat.m)$dep
      }
...
save(list(out2,out4,out6,out8), file='lmOutMarginal.rda')
...
save(list(out2c,out4c,out6c,out8c), file='lmOutCorrMarginal.rda')
