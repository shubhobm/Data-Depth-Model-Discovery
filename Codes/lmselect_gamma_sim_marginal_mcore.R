...
## overhead function
get.err = function(n.sig, X, sig, bseq, nboot, n.iter, ncores, corr=F){
  ...
  loopfun = function(b){
    ...
    for(iter in 1:n.iter){
      ...
      beta.hat = H %*% y
      r = y - X %*% beta.hat
#       samp = sample(1:n, m, replace=F)
#       H.samp = solve(t(X[samp,]) %*% X[samp,]) %*% t(X[samp,])
#       beta.hat.b =  H.samp %*% y[samp]
#       r.m = y[samp] - X[samp,] %*% beta.hat.b
      
      # Cn for full model estimates: Fn is approx by Fn^b1
      SSPmat.d = matrix(0, nrow=nboot, ncol=p+1)
      beta.mat.b = matrix(0 ,nrow=nboot, ncol=p)
      for(i in 1:nboot){
        W.i = diag(rgamma(n,b,b))
        beta.mat.b[i,] = solve(t(X) %*% W.i %*% X) %*% t(X) %*% W.i %*% y
      }
      SSPmat.d[,p+1] = mdepth.TD(beta.mat.b, beta.mat.b)$dep
      
      # marginal models
      for(j in 1:p){
        
        jbeta.mat.b = beta.mat.b      
        if(corr==T){
          jbeta.mat.b[,j] = mean(beta.mat.b[,j]) + beta.hat[j]
        ...
        SSPmat.d[,j] = mdepth.TD(jbeta.mat.b, beta.mat.b)$dep
      }
  ...
  ## Do it multicore!
  system.time(n.Cn.mat <- mclapply(bseq, loopfun, mc.cores=ncores))
  ...
  list(IC.vec = rbind(n.AIC.bk, n.AIC.all,n.BIC.bk, n.BIC.all)/n.iter,
       Cn.mat = cbind(bseq, n.Cn.mat))
}

## Run it!!
set.seed(10052015)
n = 1e2
tau.vec = seq(.1,10,by=.1)
bseq = 1/tau.vec^2
p = 10
sig = 1
nboot = 5e2
...
out2 = get.err(n.sig=2, X=X, sig=sig, bseq=bseq, nboot=nboot, n.iter=n.iter, ncores=15)
out4 = get.err(n.sig=4, X=X, sig=sig, bseq=bseq, nboot=nboot, n.iter=n.iter, ncores=15)
out6 = get.err(n.sig=6, X=X, sig=sig, bseq=bseq, nboot=nboot, n.iter=n.iter, ncores=15)
out8 = get.err(n.sig=8, X=X, sig=sig, bseq=bseq, nboot=nboot, n.iter=n.iter, ncores=15)
lmOutMarginalGamma = list(out2,out4,out6,out8)
save(lmOutMarginalGamma, file='lmOutMarginalGamma.rda')

# out2c = get.err(n.sig=2, X=X, sig=sig, bseq=bseq, nboot=nboot, n.iter=n.iter, ncores=15, corr=T)
# out4c = get.err(n.sig=4, X=X, sig=sig, bseq=bseq, nboot=nboot, n.iter=n.iter, ncores=15, corr=T)
# out6c = get.err(n.sig=6, X=X, sig=sig, bseq=bseq, nboot=nboot, n.iter=n.iter, ncores=15, corr=T)
# out8c = get.err(n.sig=8, X=X, sig=sig, bseq=bseq, nboot=nboot, n.iter=n.iter, ncores=15, corr=T)
# lmOutCorrMarginalGamma = list(out2c,out4c,out6c,out8c)
# save(lmOutCorrMarginalGamma, file='lmOutCorrMarginalGamma.rda')
